...
export function AuthPage({ onSuccess, onBack }: AuthPageProps) {
  ...
  const googleRequest = useMutation((id_token: string) => auth.loginWithGoogle({ id_token }));
  ...
  /**
   * Finish a Google sign-in. The token is only a claim until the API has
   * checked it against Google's keys, so nothing is read out of it here.
   *
   * A citizen arriving for the first time is signed in to an account that has
   * no phone number and is not verified yet — the API says so with
   * `registered`. They are let straight through, exactly like every other
   * sign-in path, and the flows that need a verified identity will still ask
   * for the phone and one-time code when the citizen reaches them.
   */
  async function handleGoogleCredential(idToken: string) {
    setFormError("");
    setNotice("");
    try {
      const session = await googleRequest.run(idToken);
      await signIn(session);
      onSuccess();
    } catch (err) {
      setFormError((err as Error).message);
              ...
              <div className="p-5">
                ...
                {mode === "login" && (
                  <form onSubmit={handleLoginSubmit} className="space-y-4">
                    ...
                    {/* Google — drawn by Google Identity Services itself */}
                    <GoogleSignInButton
                      onCredential={handleGoogleCredential}
                      disabled={submitting || googleRequest.pending}
                      locale={lang}
